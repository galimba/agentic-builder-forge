...
For how work _moves_ through these parts, see
...
_allowed_ to do is bounded by mechanical gates it cannot reason past; everything it is _asked_ to do
is a bead.
...
classes mechanical regardless of vector; tier 6 raises blast-radius isolation _when it is on_ (it is
...
| `pre-tool-use-clarify-gate.sh` | ~61   | `PreToolUse` on `AskUserQuestion`, intake only. Bounds the clarify question loop (autonomous never-ask, per-call cap, ask↔record coupling, round budget). Self-described as a real-time enhancement, _never_ the load-bearing guarantee.                                                                                    |
...
| `accept-gate.sh`                                                                                                                  | ~694        | The deterministic per-bead acceptance gate: diff ⊆ scope, DoD tests pass, success-criteria evidence present. No LLM in the verdict path. A _quality_ gate, not a deny boundary.                        |
...
- **`.claude/agents/*.md`** (support): `architect`, `builder`, `reviewer`, `disposition`,
  ...
  _the hooks are the boundary, not this prose._ See [`development.md`](development.md#roles).
...
does not require re-certification. `harness/**` files _other than_ the pre-commit hook are protected
...
The witness makes "the real hooks are loaded in _this_ session" mechanically provable, closing the
...
- **Mint:** on `SessionStart`, `session-start-witness.sh` computes the floor hash and writes
  ...
  It is **never a gate** — it exits 0 on every path. It _refuses to mint_ (leaving no witness) when
...
This is why a floor-moving change is _self-healing_: editing any of the five inputs invalidates every
