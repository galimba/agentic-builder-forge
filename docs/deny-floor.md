...
- **The off-root / bypass / not-via-`run-task.sh` intersection.** A bypass session launched where
  ...
  hook nor `permissions.deny` — the honest open limit; the witness makes a _privileged_ op fail closed.
  `[open]`.
...
floor has an entry here — so a _new_ deny rule added later without a doc entry is not caught (a deliberate v1
