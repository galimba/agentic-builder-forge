...
prefixes are benign-allowlisted by the env classifier, an agent _can_ set it on its own `finish`
...
_quality_ gate, not a deny boundary — treat this flag accordingly.
...
single-line literal ≤512 chars matched against the _staged blob_) or `sha256` (the blob's exact
...
`BD_CLOSED_WINDOW` days. Note the board is a _projection_: if `board-sync` hasn't been run recently,
...
3. **Start it.** `./harness/run-task.sh start <bead-id>`. The harness classifies it as a target build
   (distinct git store), worktrees the _target_ repo, sets the deny-hook confinement to that work
...
- **Merged-task worktrees** (`.claude/worktrees/<slug>`) — `finish` removes the sentinel and the
  container but **not** the worktree; `kill-switch` is abort-only; `reaper.sh` sweeps _containers_,
